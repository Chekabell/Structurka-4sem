...
#define COMPARE_DOUBLE 0.000001
...
//неизвестна константа MAXDOUBLE: добавить #include <float.h> и заменить MAXDOUBLE на DBL_MAX во всем коде данного файла
...
public:
    ...
    Graph           searchShortestWayABbypassingC(int from, int to, int bypassing);
    void            printPath(int from, int to);
    bool            empty();
    bool            operator !=(const Graph& src);
};
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
Graph Graph::searchShortestWayABbypassingC(int from, int to, int bypassing) {
    Graph* graphTmp = new Graph(*this);
    auto size = Size();
    //исключаем вершину bypassing из графа
    for(int i = 0; i < size; i++)
        if (graphTmp->_m[bypassing][i] > 0.0 && graphTmp->_m[bypassing][i] < DBL_MAX) {
            graphTmp->_m[bypassing][i] = DBL_MAX;
            graphTmp->_m[i][bypassing] = DBL_MAX;
        }
    //находим кратчайший путь с помощью алгоритма Дейкстры
    auto way = graphTmp->ShortestPath(from, to);
    delete graphTmp;
...
void Graph::printPath(int from, int to) {
    int V = from;
    bool** visited = (bool**)malloc(sizeof(bool*) * _Size);
    if (!visited) return;
    for (int i = 0; i < _Size; i++)
        visited[i] = (bool*)malloc(sizeof(bool*) * _Size);
    ...
    while (V != to) {
        for (int i = 0; i < _Size; i++) {
            if (_m[i][V] != 0 && _m[i][V] != DBL_MAX && !visited[i][V]) {
                std::cout << V << " -> ";
                visited[i][V] = true;
                V = i;
                break;
...
bool Graph::empty() {
    for (int i = 0; i < _Size; i++)
        for (int j = 0; j < _Size; j++)
            if (this->_m[i][j] > 0.0 && this->_m[i][j] < DBL_MAX)
                return false;
    return true;
}

bool Graph::operator !=(const Graph& src) {
    if (_Size != src._Size) return true;
    for (int i = 0; i < _Size; i++)
        for (int j = 0; j < _Size; j++)
            if (this->_m[i][j] > 0.0 && src._m[i][j] > 0.0)
                if (fabs(this->_m[i][j] - src._m[i][j]) > COMPARE_DOUBLE)
                    return true;
    return false;
}
//---------------------------------------------------------------------------
...
int main(int argc, char* argv[]){
    srand(time(NULL));
    auto noOrientedGraph = *new Graph(10);
    noOrientedGraph.Random(0.6 , 6);
    noOrientedGraph.Print();
    std::cout << std::endl;
    auto way = noOrientedGraph.searchShortestWayABbypassingC(0, 3, 7);
    if (way != noOrientedGraph) {
        way.Print();
        way.printPath(0, 3);
    }
    _getch();
